...
async function executeTool(name, input) {
  switch (name) {
    ...
    case 'get_order_status': {
      const raw = (input.order_number || '').trim().toUpperCase();
      // Accept both PS100422 and PS-100422
      const normalized = raw.replace(/^PS(\d{6})$/, 'PS-$1');
      const order = MOCK_ORDERS[normalized] || MOCK_ORDERS[raw];
      if (!order) {
        return {
          found: false,
      ...
      return { found: true, order_number: normalized || raw, ...order };
    }
