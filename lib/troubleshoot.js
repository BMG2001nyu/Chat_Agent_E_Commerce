...
const DIAGNOSTIC_FLOWS = {
  ...
  dishwasher: [
    ...
    {
      patterns: ['not cleaning', 'dirty dishes', 'dishes not clean', 'food residue', 'not washing properly', 'dishes not getting clean', 'not getting clean', 'dishes coming out dirty', 'not coming clean', 'spots on dishes'],
...
function troubleshootIssue({ symptom, appliance_type, model_number }) {
  ...
  // Require a meaningful match — lone word coincidences (score < 4) fall through to fallback
