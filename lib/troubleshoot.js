...
const DIAGNOSTIC_FLOWS = {
  refrigerator: [
    ...
    {
      patterns: ['not cooling', 'warm fridge', 'fridge warm', 'refrigerator warm', 'not cold', 'stopped cooling', 'not keeping cold'],
    ...
    {
      patterns: ['freezer cold refrigerator warm', 'fridge section warm', 'freezer works refrigerator doesnt', 'refrigerator compartment warm'],
  ...
  dishwasher: [
    ...
    {
      patterns: ['not drying', 'wet dishes', 'dishes still wet', 'not heating', 'cold water'],
...
function troubleshootIssue({ symptom, appliance_type, model_number }) {
  ...
  const { parts: _, ...rest } = require('../data/parts.json');
