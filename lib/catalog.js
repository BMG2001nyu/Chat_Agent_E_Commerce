...
// PS-11752778 → PS11752778, 11752778 → PS11752778
function normalizePartNumber(pn) {
  if (!pn || typeof pn !== 'string') return pn;
  let n = pn.trim().toUpperCase().replace(/\s+/g, '');
  n = n.replace(/^PS-(\d+)$/, 'PS$1');
  if (/^\d+$/.test(n)) n = 'PS' + n;
  return n;
}
...
function getPartByNumber(part_number) {
  const normalized = normalizePartNumber(part_number);
  return parts.find(p => p.part_number === normalized) || null;
}
...
module.exports = { searchParts, getPartByNumber, checkCompatibility, getPartsBySymptom, normalizePartNumber };
